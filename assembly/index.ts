import { context, ContractPromiseBatch, datetime, PersistentMap, PersistentVector, u128 } from "near-sdk-as";
import { Constants, Service, Subscriber } from "./models";
import { AccountId, BASE_TO_CONVERT, toYoctob128 } from "./utils";
...
export class SubService {
  ...
  private _nearToYocto(x: string): u128 {
   ...
   */
  @mutateState()
  addService(title: string, discription: string, provider: AccountId, profitPerRedeem: string, expiryDate: string): Service {
  ...
  subscribe(): Subscriber {
    ...
    assert(context.attachedDeposit == Constants.SUB_FEES, Constants.formulateWrongeDepositMessage(context.attachedDeposit));
    let currentTimestamp: u64 = this._toMillisecond(context.blockTimestamp);
  ...
  redeem(serviceId: string): Service {
    ...
    service.redeemCount++;
    ContractPromiseBatch.create(service.provider).transfer(this._nearToYocto(service.profitPerRedeem));
